---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the wiswa-mcp project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'wiswa.mcp': {}})
  ```
...
- `` :py:mod:`wiswa.mcp.string` `` for modules
- `` :py:func:`wiswa.mcp.string.slugify` `` for functions
- `` :py:class:`wiswa.mcp.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~wiswa.mcp.string.slugify` `` renders as `slugify`
